...
impl QueryEngine {
    ...
    pub fn impact(
        ...
        let mut impacted_nodes: Vec<_> = impacted
            .iter()
            .filter_map(|id| self.state.graph.nodes.get(id))
            .cloned()
            .collect();
        impacted_nodes.sort_by(|a, b| a.id.cmp(&b.id));

        let mut supporting_edges: Vec<_> = supporting_edges
            .iter()
            .filter_map(|id| self.state.graph.edges.get(id))
            .cloned()
            .collect();
        supporting_edges.sort_by(|a, b| a.id.cmp(&b.id));
